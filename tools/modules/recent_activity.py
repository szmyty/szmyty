...
def _summarize_event(event: dict[str, Any]) -> tuple[str, str] | None:
    ...
    repo_url = (
        f"https://github.com/{repo_name}" if repo_name else "https://github.com/szmyty"
    )
...
def fetch_live_activity(
    ...
    payload, _ = _github_get_json(
        f"{API_ROOT}/users/{username}/events/public?per_page=20", token
    )
...
def build_activity(
    output_path: Path, fixture_path: Path = DEFAULT_FIXTURE, token: str | None = None
) -> RecentActivity:
        ...
        if cached is not None:
            ...
            try:
                ...
                raise ProviderFailure(
                    f"No usable recent activity data: {exc};"
                    f" fixture failed: {fixture_exc}"
                ) from fixture_exc
    _write_json(output_path, activity.model_dump(mode="json"))
    return activity
