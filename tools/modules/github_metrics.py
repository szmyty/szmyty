...
def select_maintained_repositories(repositories: list[dict[str, Any]]) -> list[RepositorySummary]:
...
def main(output_path: Path, fixture_path: Path) -> None:
    ...
    metrics = build_metrics(output_path=output_path, fixture_path=fixture_path, token=token)
