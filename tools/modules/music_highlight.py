...
def build_music_highlight(
    ...
    try:
        ...
        if cached is not None:
            ...
            try:
                music = load_music(fixture_path).model_copy(
                    update={"data_source": "fixture"}
                )
            except Exception as fixture_exc:  # noqa: BLE001
                raise ValueError(
                    f"No usable music highlight data: {exc};"
...
def main(input_path: Path, output_path: Path, fixture_path: Path) -> None:
    ...
    music = build_music_highlight(
        input_path=input_path, output_path=output_path, fixture_path=fixture_path
    )
